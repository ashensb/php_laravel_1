...
<div class="container-fluid px-4 py-4" style="background-color:#0f1115; min-height:100vh;">
    ...
    <!-- Header Section with Export Option -->
    <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center mb-4 pb-3" style="border-bottom:1px solid #23262f;">
        <div>
            ...
            <p class="small mb-0" style="color:#8b93a1;">Review, grade, and export results for completed exams.</p>
        </div>

        <!-- Export Report Dropdown / Form -->
        @if(isset($teacherExams) && $teacherExams->count() > 0)
        <div class="mt-3 mt-md-0 d-flex align-items-center gap-2">
            <select id="exportExamSelect" class="form-select form-select-sm" style="background:#161a22; border:1px solid #334155; color:#f1f3f7; max-width:250px;">
                <option value="">-- Select Exam to Export --</option>
                @foreach($teacherExams as $tExam)
                    <option value="{{ $tExam->id }}">{{ $tExam->title }}</option>
                @endforeach
            </select>
            <button onclick="downloadReport()" class="btn btn-sm px-3 fw-semibold" style="background:#059669; color:#ffffff; border:none;">
                ...
                <tbody>
                    @forelse($submissions as $sub)
                        <tr>
                            ...
                            <td style="color:#34d399;" class="fw-bold">{{ $sub->score ?? 'Pending' }}</td>
                            <td>
                            ...
                            <td style="color:#8b93a1;">{{ \Carbon\Carbon::parse($sub->submitted_at)->format('Y-m-d h:i A') }}</td>
                            <td class="pe-3">
                                ...
                                <a href="{{ route('teacher.exams.export_report', $sub->exam_id) }}" class="btn btn-sm btn-outline-success ms-1" title="Export this exam report">
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="text-center py-4" style="color:#8b93a1;">No submissions found.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
...
function downloadReport() {
    var examId = document.getElementById('exportExamSelect').value;
    if(!examId) {
        alert('Please select an exam first to export the report.');
        return;
    }
    window.location.href = "/teacher/exams/" + examId + "/export-report";
}
</script>
@endsection